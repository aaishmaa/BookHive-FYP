...
import {
  RefreshCcw, Check, X, TrendingUp, Clock,
  MessageCircle, Loader, ExternalLink, ArrowLeftRight,
} from "lucide-react";
          ...
          {[
            ...
            <button key={t.key} onClick={() => { setTab(t.key); setFilter("All"); }}
              className={`flex items-center gap-2 px-1 pb-3 mr-6 text-[13.5px] font-semibold border-b-2 transition
                ${tab === t.key ? "border-[#1C7C84] text-[#1C7C84]" : "border-transparent text-gray-500 hover:text-gray-700"}`}>
              {t.label}
              {t.count > 0 && (
            ...
            <AnimatePresence>
              {filtered.length > 0 ? filtered.map((req, i) => {
                        ...
                        {isExchange && (req.offerBookTitle || req.offerBookImg) ? (
                            ...
                            <div className="flex items-center gap-1.5 flex-1 min-w-0">
                              {req.bookImg && (
                                <img src={req.bookImg} alt={req.book}
                                  className="w-8 h-10 rounded object-cover shrink-0 border border-gray-200" />
                              )}
                              <div className="min-w-0">
                                <p className="text-[10px] text-gray-400 font-medium">{tab === "received" ? "They want" : "You want"}</p>
                                <button onClick={() => req.bookId && navigate(`/book/${req.bookId}`)}
                                  className="text-[11.5px] font-semibold text-[#1C7C84] hover:underline truncate block max-w-[100px]">
                                  {req.book}
                                </button>
                              </div>
                            </div>
                            ...
                            <div className="flex items-center gap-1.5 flex-1 min-w-0">
                              {req.offerBookImg && (
                                <img src={req.offerBookImg} alt={req.offerBookTitle}
                                  className="w-8 h-10 rounded object-cover shrink-0 border border-emerald-200" />
                              )}
                              <div className="min-w-0">
                                <p className="text-[10px] text-emerald-600 font-medium">{tab === "received" ? "Offering" : "You offer"}</p>
                                <button onClick={() => req.offerBookId && navigate(`/book/${req.offerBookId}`)}
                                  className="text-[11.5px] font-semibold text-gray-800 hover:text-[#1C7C84] truncate block max-w-[100px]">
                                  {req.offerBookTitle}
                                </button>
                              </div>
                            </div>
                          ...
                          <p className="text-[12.5px] text-gray-600 mt-1">
                            ...
                            <button onClick={() => req.bookId && navigate(`/book/${req.bookId}`)}
                              ...
                              "{req.book}" <ExternalLink className="w-3 h-3" />
                            </button>
                          </p>
                        )}
                        ...
                        {req.status === "Accepted" && isExchange && (
                          <div className="mt-2 bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-2">
                            <p className="text-[11.5px] text-emerald-700 font-semibold">
                              ✅ Exchange accepted — both books marked as exchanged!
                            </p>
                          </div>
                        )}
                    ...
                    <div className="flex items-center gap-2 mt-4 flex-wrap">
                      {tab === "received" && req.status === "Pending" && (
                        <>
                          ...
                          <button onClick={() => handleUpdate(id, "Declined")} disabled={updatingId === id}
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                  ...
                  {tab === "sent" && <p className="text-[12.5px] mt-1">Browse books and send a request</p>}
                </motion.div>
              )}
            </AnimatePresence>
          </div>
