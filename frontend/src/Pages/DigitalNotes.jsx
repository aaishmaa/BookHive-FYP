    ...
    <motion.div
      ...
      <div className="px-4 pb-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button onClick={() => setLiked(!liked)} className="flex items-center gap-1 text-gray-400 hover:text-red-500 transition">
...
const DigitalNotes = () => {
  ...
  const recentUploads = notes.slice(0, 3).map(n => ({ title: n.title, time: timeAgo(n.createdAt) }));

  return (
    <div className="flex h-full overflow-hidden bg-gray-50">
      <div className="flex-1 overflow-y-auto px-6 py-6">
        <motion.h1 initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.3 }} className="text-2xl font-bold text-gray-900 mb-5">
          Digital Notes
        </motion.h1>
          ...
          {TABS.map(t => (
            <button key={t} onClick={() => setActiveTab(t)}
              className={`px-4 py-1.5 rounded-full text-[13px] font-medium border transition whitespace-nowrap
                ${activeTab === t ? "bg-[#1C7C84] text-white border-[#1C7C84]" : "bg-white text-gray-600 border-gray-200 hover:border-[#1C7C84] hover:text-[#1C7C84]"}`}>
              {t}
            </button>
          ))}
        </div>
